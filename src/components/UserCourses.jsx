...
export default function UserCourses() {
    ...
    const fetchEnrolledCourses = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          throw new Error(
            response.data.error || "Failed to fetch enrolled courses"
          );
        }
      } catch (err) {
  ...
  const handleUnenroll = async (courseId, courseName) => {
    if (
      !window.confirm(`Are you sure you want to unenroll from "${courseName}"?`)
    ) {
      return;
    }
          ...
          <h3>
            {
              enrolledCourses.filter(
                (course) => course.enrollment.progress === 100
              ).length
            }
          </h3>
          <p>Completed</p>
                ...
                {filteredCourses.map((course) => (
                  <div
                    ...
                    className={`course-card ${
                      course.enrollment.progress === 100 ? "completed" : ""
                    }`}
                  >
                    ...
                    <div className="course-actions">
                      <Link
                        to={`/courses/${course._id}/view`}
                        className="course-button primary"
                      >
                        {course.enrollment.progress > 0 ? "Continue" : "Start"}{" "}
                        Course
                      </Link>
                      <Link
                        to={`/courses/${course._id}/resources`}
