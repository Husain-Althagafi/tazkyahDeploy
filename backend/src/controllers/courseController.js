...
exports.adminEnrollStudent = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== "admin") {
    return res
      .status(403)
      .json({ error: "Only administrators can use this endpoint" });
  }
...
exports.unenrollStudentFromCourse = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== "student") {
    return res
      .status(403)
      .json({ error: "Only students can unenroll from courses" });
  }

  // Unenroll student from course using repository
  const result = await courseRepository.unenrollStudent(
    req.user.id,
    course._id
  );
...
exports.getEnrolledStudents = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    return res
      .status(403)
      .json({ error: "Not authorized to view enrolled students" });
  }
...
exports.getStudentCourses = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== "student") {
    return res
      .status(403)
      .json({ error: "Only students can access enrolled courses" });
  }
